...
def betweenness_centrality(G, k=None, normalized=True, weight=None,
    ...
    if k is None:
        ...
        else: # use given access points for calculation
            nodes = k
    ...
    for s in nodes:
        ...
        if i >= progress:
            ...
            txt = 'Betweenness computation %s %% sor far in %s' %(percentage, GetTextTime(tf))
...
def edge_betweenness_centrality(G, k=None, normalized=True, weight=None, seed=None):
    ...
    for s in nodes:
        ...
        if i >= progress:
            ...
            txt = 'Betweenness computation %s %% sor far in %s' %(percentage, GetTextTime(tf))
            wx.CallAfter(Publisher().sendMessage, "consoleAppend", txt)
    wx.CallAfter(Publisher().sendMessage, "consoleAppend", 'Rescaling... Remove nodes to only return edges. Please be patient...')
    # rescaling
...
def _rescale(betweenness,n,normalized,directed=False,k=None):
    ...
    if scale is not None:
        if k is not None:
            if isinstance(k, int):
                scale=scale*n/k
            else: # use number of given access points
                scale=scale*n/len(k)
